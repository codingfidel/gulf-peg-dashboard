...
# FX Reserves Plot
st.subheader("FX Reserves")
fig_res = go.Figure()
fig_res.add_trace(go.Scatter(x=df["Year"], y=df["UAE_Reserves"], name="UAE Reserves"))
fig_res.add_trace(go.Scatter(x=df["Year"], y=df["KSA_Reserves"], name="KSA Reserves"))
fig_res.add_trace(go.Scatter(x=df["Year"], y=df["Qatar_Reserves"], name="Qatar Reserves"))
fig_res.update_layout(height=400, width=1000)
...
st.markdown("Source: BIS, IMF, national banks (compiled and modeled)")
